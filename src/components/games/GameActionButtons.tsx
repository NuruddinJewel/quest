...
export default function GameActionButtons({ game }: ActionButtonsProps) {
    ...
    const handleInstantBuy = async () => {
        ...
        try {
            ...
            console.error("Order failed:", error);
            // setErrorMessage(error.message || "Something went wrong while securing your order.");
        } finally {
            setIsSubmitting(false);
        }
    };
        ...
        <div className="space-y-4 pt-4 border-t border-gray-800/50">
            ...
            {orderSuccess && (
                <div className="bg-emerald-500/10 border border-emerald-500/35 rounded-xl p-4 flex items-center gap-3 animate-in fade-in slide-in-from-top-4 duration-300">
                    <HiCheckCircle className="text-emerald-400 text-2xl shrink-0" />
                    <div>
                        <p className="text-sm font-bold text-ivory">ORDER REQUESTED SUCCESSFULLY!</p>
                        <p className="text-xs text-fog mt-0.5">Awaiting admin approval. Checked out as <span className="text-cyan">{session?.user.email}</span>.</p>
                    </div>
                </div>
            )}
            ...
            {/* Error Message Toast */}
            {errorMessage && (
                <div className="bg-rose-500/10 border border-rose-500/35 rounded-xl p-4 text-rose-400 text-sm font-medium animate-in fade-in slide-in-from-top-4 duration-300">
                    ❌ {errorMessage}
                </div>
            )}
            ...
            <div className="flex flex-col sm:flex-row gap-4">
                ...
                >
                    <HiCreditCard className="text-lg" />
                    <span>
                        {isPending ? "Checking Session..." : isSubmitting ? "Securing Order..." : outOfStock ? "Out of Stock" : "Buy Now"}
                    </span>
                </button>
            </div>
        </div>
